...
export function CartBar() {
  const { totalItems, totalGuaranies } = useCart();
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-20 bg-white border-t border-tylo-teal/10 shadow-[0_-4px_20px_rgba(13,92,92,0.08)]">
      <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <Link
          href="/carrito"
          className="flex-1 max-w-[200px] py-3 px-4 rounded-lg bg-tylo-teal text-white text-center font-medium text-sm shadow-sm hover:bg-tylo-teal-dark transition-colors"
        >
          Ver carrito
        </Link>
      </div>
